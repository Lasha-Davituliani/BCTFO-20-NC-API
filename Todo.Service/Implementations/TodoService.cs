...
namespace Todo.Service.Implementations
{
    public class TodoService : ITodoService
    {
        ...
        public async Task DeleteTodoAsync(string userId, int id)
        {
            ...
            userId = _todoRepository.LoginRe
        ...
        public async Task<List<TodoForGettingDto>> GetUserTodosAsync(string userId)
        {
            if (userId is null)
                throw new ArgumentException("Invalid argument passed");
            var row = await _todoRepository.GetAllTodosAsync(x => x.UserId == userId);
            if (row == null)
                throw new TodoNotFoundException();

            var result = _mapper.Map<List<TodoForGettingDto>>(row);
            return result;
        }
        ...
        public async Task<TodoForGettingDto> GetSingleTodoAsync(string userId, int id)
        {
            if(userId is null || id  <= 0)
                throw new ArgumentException("Invalid argument passed");

            var raw = await _todoRepository.GetSingleTodoAsync(x => x.UserId == userId && x.Id == id);
            if (raw == null)
                throw new TodoNotFoundException();
        ...
        public async Task AddTodoAsync(TodoForCreatingDto model, string userId)
        {
            if (model is null || userId is null)
                throw new ArgumentNullException("Invalid argument passed");
